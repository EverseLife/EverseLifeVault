...
#: Узел столицы: с D-330 город стоит на своём биопринтере, и пустого узла
#: над ним больше нет. Ключ здесь один и записан прямо, потому что искать
#: его нечем — `pins` читает файл, а не сборку, и «узел с городом» пришлось
#: бы разбирать вторым чтением раскладки ради одного имени.
CAPITAL = "terra.capital.core"

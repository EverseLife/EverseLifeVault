...
DIFFUSION = 0.03
#: 500 → 125 при ужатии планет 2026-09-10, 125 → 62,5 при втором (D-329):
#: это единственная **абсолютная длина** во всём файле, а мерит она ландшафт
#: — длину, на которой склон оплывает. Ландшафт ужался вместе с планетой
#: (формы держат угловой размер, см. `terrain.relief_m` в реестре), значит и
#: она делится на то же, на что делится радиус. Проверка: путь оплывания за
#: прогон, `sqrt(итераций · kd) · сторона`, был 420–840 м у Терры до первого
#: ужатия, 105–210 после него и 52–105 теперь — ровно во столько же раз.
DIFFUSION_STEP_M = 62.5

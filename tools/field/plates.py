...
REACH_R = 1.2
#: Сколько раундов среднего по соседям сглаживают схождение, разошедшееся
#: по плите. Схождение считается **по клетке** границы — по её нормали к
#: соседу с той стороны, — а `spread` отдаёт каждой клетке плиты значение
#: **ближайшей** клетки границы, то есть кроит всю полосу хребтов и рифтов
#: на области Вороного клеток границы: на шве двух областей схождение
#: прыгает ступенью, и шов между соседними клетками — прямая под сорок пять
#: градусов по решётке. Рифт и хребет несли ступень в высоту: на Терре
#: западнее столицы дно у берега падало на 37 м вдоль одного столбца
#: текселей, и кромка моря на полтора километра шла по этой прямой
#: (владелец 2026-09-11: «на карте встречаются ромбы»). Усреднение вдоль
#: самой границы не помогает — ступени стоят между **разными** отрезками
#: границы; помогает размыть разошедшееся поле: шестнадцать раундов — сигма
#: около трёх клеток (`Grid.blur`), полтораста метров на Терре против
#: полутора километров ширины рифта. Худшая сотая доля шагов между соседями
#: на сетке теста падает с 1,8 размаха схождения до 0,37
#: (`test_the_convergence_spreads_from_the_boundary_without_steps`).
CONVERGENCE_BLUR = 16
...
def build(
    ...
    convergence = grid.blur(convergence, CONVERGENCE_BLUR)

...
#: Потолок имени города. Узел с `city: true` — это будущий город: сид зовёт на
#: нём `city.found`, и имя узла становится именем города, а оно, в свою очередь,
#: именем официального канала города в сети. У канала свой потолок, и `создать`
...
def check_world(doc: dict, recipes_doc: dict, all_recipes) -> list[str]:
    ...
    for node in doc.get("nodes") or []:
        ...
        if node.get("city"):
            ...
            #: Имя города приезжает отсюда: сид основывает город именем узла,
            #: и оно же становится именем канала. Ловится здесь, а не отказом
            #: движка: это дефект содержания, и говорить о нём надо тому, кто
            #: правит вольт, а не игроку в окне.
            title = node.get("name")
            #: Не строка — своя жалоба, а не `len` по чему попало: `name: 2026`
            #: разберётся в int, пройдёт проверку «без имени» и уедет в
            #: `City.name` целым числом (`seed_world.load_scenario` кладёт
            #: `name` как есть).

...
#: Потолок имени города. Узел с `city` — это будущий город: сид зовёт на нём
#: `city.found` этим самым именем, а оно, в свою очередь, становится именем
#: официального канала города в сети. У канала свой потолок, и `создать`
#: откажет любому игроку, кто наберёт длиннее, — то есть слишком длинное имя в
#: раскладке молча выдало бы город с каналом, который сеть сама бы не приняла.
#:
#: **Число здесь — копия.** В движке это `CITY_NAME_LIMIT` (`src/runtime.py`),
#: и импортировать его сюда неоткуда: сборка читает `data/`, а из вольта в
#: репозиторий игры уезжает только `build/*.json` — питон не уезжает. Общего у
#: сторон одна раскладка, по ней и сверяются: `test_seed` читает
#: `build/world.json` и меряет имена городов своим `CITY_NAME_LIMIT`.
#:
#: Что эта пара ловит, а что нет. Ловит вред: имя, переросшее меньший из двух
#: потолков, роняет одну из сторон, какая бы из них ни разъехалась. Не ловит
#: сам разъезд: пока содержание до меньшего потолка не доросло, числа могут
#: расходиться молча. Закрыть и это можно, отдав потолок в выход сборки —
#: полем в `build/world.json`, который CI и так копирует, — тогда движок
#: сверял бы само число. Не сделано: решать, что лежит в снимке раскладки,
#: — не дело проверки длины.
WORLD_CITY_NAME_LIMIT = 40
...
def check_spacing(doc: dict, constants: dict) -> list[str]:
    ...
    #: Узел-пустышка в сравнение не идёт: это не место, куда ходят, а якорь
    #: группы (площадь у него метр), и своих детей он носит поверх себя —
    #: первый из них прибит к `{x: 0, y: 0}`, то есть ровно в ту же точку.
    #: Объявлять это теснотой значило бы жаловаться на замысел: карта рисует
    #: либо свёрнутую группу, либо её нутро, и вместе они в кадр не попадают.
    #:
    #: Пустышка — та, что **не город** (D-330). У живого города такого узла
    #: больше нет: имя носит сам биопринтер, и он — обычное место, которое в
    #: кадре стоит рядом с рынком и мастерской. Он в сравнении есть, и это тот
    #: самый узел, вокруг которого стоит весь город. Из групп-пустышек
    #: остались одни мёртвые города Предтеч: их нутро карта показывает вместо
    #: них самих, вместе они в кадр не попадают.
    holds = {
        node.get("parent")
        for node in doc.get("nodes") or []
        if node.get("parent")
    } - {node["key"] for node in doc.get("nodes") or [] if node.get("city")}
    #: Узел **без** своего места в сравнение попасть не может: сид его не
    #: пинует, а усаживает по спирали рядом с занятыми
    #: (`places.assign`), и где он встанет, до сида не знает никто. Молча
    #: пропускать такие было дырой ровно в том узле, вокруг которого стоит
    #: весь город: биопринтер писался без смещения, проверка его не видела —
    #: и «Администрация» в двадцати одном метре от него проходила молча при
    #: зазоре в двадцать восемь. Поэтому не пропуск, а отказ: раскладка,
    #: часть которой решает сид, не раскладка.
    blind = [
        node["key"]
        for node in doc.get("nodes") or []
        if node.get("place") is None
        and (node.get("anchor") or node.get("parent"))
        and node["key"] not in holds
        and "." in node["key"]
    ]
    ...
    return [
        f"мир: у «{key}» нет своего места — сид усадит его вслепую, и"
        f" проверка зазора его не увидит"
...
def check_world(doc: dict, recipes_doc: dict, all_recipes) -> list[str]:
    ...
    for node in doc.get("nodes") or []:
        ...
        #: Город — это имя, а не галочка (D-330). Носит его тот же узел, что
        #: стоит на карте своим: пустого узла площадью в метр под живым
        #: городом больше нет, и «Столица Терры» принадлежит биопринтеру,
        #: вокруг которого город стоит. Имя приезжает отсюда — сид основывает
        #: город этим словом, и оно же становится именем канала, — потому
        #: ловится здесь, а не отказом движка: это дефект содержания, и
        #: говорить о нём надо тому, кто правит вольт, а не игроку в окне.
        title = node.get("city")
        if title is not None:
            if layer != "planet":
                problems.append(f"мир: город «{key}» — не узел слоя planet")
            #: `city: true` — прежняя запись, когда имя брали у узла. Своя
            #: жалоба, а не `len` по булеву: она говорит, что делать.
            if not isinstance(title, str):
                problems.append(
                    f"мир: у «{key}» в `city` не имя города, а {title!r} — "
                    f"городом теперь помечает имя, а не галочка (D-330)"
                )
            elif not title.strip():
                problems.append(f"мир: имя города «{key}» — пустое")
            else:

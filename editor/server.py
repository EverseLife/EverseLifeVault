...
class Session:
    ...
    def __init__(self, vault_root: Path):
        ...
        self.world = vault_root / "data" / "world.yaml"
...
def world_node(session: Session, query: dict, body: dict) -> dict:
    ...
    with session.lock:
        ...
        file.save(lines, doc)
    return {"key": body.get("key"), "check": _check(session)}
...
def world_node_delete(session: Session, query: dict, _body: dict) -> dict:
    ...
    with session.lock:
        ...
        file.save(lines, doc)
    return {"key": key, "check": _check(session)}
...
def world_edge(session: Session, _query: dict, body: dict) -> dict:
    with session.lock:
        ...
        file.save(lines, doc)
    return {"a": body.get("a"), "b": body.get("b"), "check": _check(session)}


def world_edge_delete(session: Session, query: dict, _body: dict) -> dict:
    a, b = _need(query, "a"), _need(query, "b")
    with session.lock:
        file = session.open_world()
        lines, doc = file.drop_edge(a, b)
        file.save(lines, doc)
    return {"a": a, "b": b, "check": _check(session)}
...
def world_pocket(session: Session, query: dict, body: dict) -> dict:
    ...
    owner = _need(query, "owner")
    with session.lock:
        file = session.open_world()
        lines, doc = file.put_pocket(owner, body.get("items") or [])
        file.save(lines, doc)
    return {"owner": owner, "check": _check(session)}
...
def undo(session: Session, _query: dict, _body: dict) -> dict:
    ...
    #: the tab in front of the person: the editor writes three of them now --
    #: recipes, building types (D-218) and the world's layout (D-243). The
    #: backup's own name says which, and they all lie in `data/`.
    with session.lock:

...
**Громкость и выключатель** — кнопка-нота в шапке, под ней ползунок и
кнопка «выключить». Это настройка вида по D-298: живёт в браузере
(`everselife.music.volume`, `everselife.music.muted`), не в учётной записи,
переживает перезагрузку и выход из игры. Два ключа, а не один: включение
возвращает прежний уровень, а не ноль. Ползунок включает музыку сам
(сдвинул — значит хочет слышать); выключает только кнопка — случайный
сдвиг не заглушит игру. **По умолчанию включено на 50%** (владелец,
2026-09-12). Слух получает квадрат положения ползунка: линейная шкала на
слух неровная.
...
**Луп** — атрибутом `loop` одного элемента через Web Audio; у Chrome на
стыке есть щелчок в миллисекунды, и он маскируется, если резать луп в
тихом месте трека. Смена трека — спад за 1,2 с, подмена, подъём.
